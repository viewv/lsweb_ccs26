...
// Note: use the lsweb-wrapper
import { ZMQWrapper } from "./zmq-lsweb-wrapper";
...
const fetchSession = async () => {
    const zmqSession = new ZMQWrapper();
    ...
    if (totalWorkers > 0) {
        ...
        if (workersInactive === 0) {
            ...
            if (session) {
                ...
                const landing_page = session.session_information.website.url;
                const rank = session.session_information.website.rank;
                ...
                try {
                    // TODO  test the session attached without session data
                    await SubjectFactory.createSubjectFromUrlString(landing_page, 0, additionalInfo, undefined, undefined, session, rank)
                } catch (err: unknown) {
                    console.log(err);
                    Logging.error(`Failed to create new subject for "${landing_page}" without context.`);
                }
