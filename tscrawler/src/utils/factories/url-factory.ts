...
class URLFactory {
     ...
     * @returns 
     */
    static async createUrlFromString(url: string, depth: number, parent?: number, session?: Session, transaction?: Transaction, rank?: number): Promise<Url | undefined> {
        // Calculate a URL hash to quickly check whether relevant url is also present (with normalized value, so stuff 
        // like query parameters are ignored in hash).
        const url_hash: string = md5(prepareUrl(url))
        try {
            ...
            if (!urlInDb) {
                ...
                if (parent) {
                    ...
                    //  add the ranking infomation
                    if (rank) {
                        domain = await DomainFactory.createDomainFromUrl(url, 0, rank, session?.id, transaction);
                    } else {
