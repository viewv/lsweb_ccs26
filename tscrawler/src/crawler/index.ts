...
class Crawler {
  ...
  async visit(work: Task) {
    ...
    try {
      ...
      //  change into check the session data
      if (element.session?.session_data) {
      ...
      Logging.info(`Load all modules from modules folder into the crawler and store in modules array`)

      // Load all modules from modules folder into the crawler and store in modules array
      Logging.info("Fetching specified module")
      const files = fs.readdirSync(path.join(__dirname, "..", "modules"));
      const imports = await Promise.all(files.map(file => (
        import(path.resolve(__dirname, '..', 'modules', file)).then((module: ImportModuleType) => module.default)))
      )
      ...
      Logging.info("Registering task in module")
      for (let index = 0; index < this.modules.length; index++) {
        const module = this.modules[index];
      ...
      Logging.info("Executing before() method of module")
      ...
      Logging.info("Opening URL that was specified via page.goto")
      ...
      Logging.info("Waiting for page to finish executing")
      ...
      if (config.links.collect && subject.type === SubjectType.RECONNAISSANCE) {
        Logging.info("Retrieving all on page links from document.links")
        ...
        Logging.info("Skipping link collection due to configuration");
        Logging.info("collect:" + config.links.collect + subject.type);
      }
