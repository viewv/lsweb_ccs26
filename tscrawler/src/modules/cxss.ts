...
class CXSS extends Module {
    ...
    setup = async () => {
        if (config.mode === "connected") {
            ...
            await sequelize.query("CREATE TABLE cxss_exploit (exploit_id SERIAL PRIMARY KEY, finding_id INTEGER REFERENCES cxss_finding(finding_id) ON DELETE CASCADE, type TEXT, status INTEGER, exploit_data JSON, session_id INTEGER REFERENCES sessions(id), created_at TIMESTAMP, updated_at TIMESTAMP)");
                ...
                for (let index = 0; index < this.findings.length; index++) {
                    ...
                    for (let j = 0; j < this.frames.length; j++) {
                        ...
                        if (url == frame.endUrl && frame.main) {
                            ...
                            let testSinkId = sinkToId(element.sink)
                            if (testSinkId == -1) {
                                                ...
                                                if (type === ExploitType.RCXSS) {
                                                    ...
                                                    for (let j = 0; j < Math.min(100, exploit_url.length); j++) {
                                                        ...
                                                        await Subject.create({
                                                            ...
                                                            //TODO  Try to fix the bugs maybe not right
    ...
    async reportExploitVerification(exploitId: number, success: boolean, console: string[]) {
        if (config.mode === "connected") {
            ...
            //  DONOT: Store console output for exploit on disk
            ...
            if (success) {
                Logging.info(`[cxss] Exploit ${exploitId} successfully verified.`)
                try {
                    // Get finding related information to construct path
                    const finding = await sequelize.query(
                        'SELECT e.finding_id, f.frame_id FROM cxss_exploit e JOIN cxss_finding f ON e.finding_id = f.finding_id WHERE e.exploit_id = :exploitId',
                        {
                            replacements: { exploitId },
                            type: QueryTypes.SELECT
                        }
                    );

                    if (finding.length > 0) {
                        const findingId = (finding[0] as any).finding_id;
                        const frameId = (finding[0] as any).frame_id;
                        // Delete the corresponding exploit-generator directory
                        const generatorOutPath = path.join(config.dataPath, "exploit-generator", String(this.reportId), String(frameId), String(findingId));
                        if (fs.existsSync(generatorOutPath)) {
                            fs.rmSync(generatorOutPath, { recursive: true, force: true });
                            Logging.debug(`[cxss] Successfully deleted payload data for finding id="${findingId}" after verification`);
                        }
                    }
                } catch (err: any) {
                    Logging.error(`[cxss] Failed to delete payload data after verification. Error: ${err.toString()}`);
                }
            } else {
                Logging.info(`[cxss] Exploit ${exploitId} verification failed.`)
            }
        } else {
